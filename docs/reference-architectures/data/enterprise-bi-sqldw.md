...
可能的話將在離峰時間進行資料擷取，以避免影響生產環境的效能。 

請避免在資料庫伺服器上執行 bcp，改以其他機器來執行。 請將檔案寫入至本機磁碟機。 請確定您有足夠的 I/O 資源可處理並行寫入。 為了達到最佳效能，請將檔案匯出至專用的快速儲存磁碟機。

您可以用 Gzip 壓縮格式儲存匯出的資料，以加快網路傳輸速度。 請注意，被壓縮的檔案載入至倉儲的速度會低於不被壓縮檔案的載入速度，因此，您須在網路傳輸速度與載入速度做出取捨。 如果您決定使用 Gzip 壓縮，請勿建立單一 Gzip 檔案，請將資料分成多個壓縮檔案。

### <a name="copy-flat-files-into-blob-storage"></a>將一般檔案複製到 Blob 儲存體中

[AzCopy](/azure/storage/common/storage-use-azcopy) AzCopy utility可讓您以高效能將資料複製到 Azure Blob 中。

**建議**

儲存體帳戶請建立在來源資料所在位置的最近區域中。 請將儲存體帳戶和 SQL 資料倉儲執行個體部署在相同區域中。 
...
AzCopy 會透過公用網際網路將資料移至儲存體。 若其速度不夠快，請考慮設定 [ExpressRoute](/azure/expressroute/) 線路。 ExpressRoute 是一項服務，它會透過專用私人連線將您的資料路由傳送至 Azure。 當網路連線速度太慢時，你也可以考慮將磁碟上的資料傳送至 Azure 資料中心。 如需詳細資訊，請參閱[從 Azure 來回傳輸資料](/azure/architecture/data-guide/scenarios/data-transfer)。
...
如果您有大量資料 (超過 1 TB)，且您分析工作負載將透過平行處理來執行，請考慮使用 SQL 資料倉儲。 SQL 資料倉儲不適用於 OLTP 工作負載或較小的資料集 (< 250 GB)。 對於小於 250 GB 的資料集，請考慮使用 Azure SQL Database 或 SQL Server。 如需詳細資訊，請參閱[資料倉儲](../../data-guide/relational-data/data-warehousing.md)。
