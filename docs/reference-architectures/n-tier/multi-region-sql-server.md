...
- **健康情況探查**。 流量管理員會使用 HTTP (或 HTTPS) [探查][tm-monitoring]來監視每個區域的可用性。 探查會針對指定的 URL 路徑檢查 HTTP 200 回應。 最佳作法是，建立端點來報告應用程式的整體健康情況，並使用此端點進行健康情況探查。 否則，探查可能會在應用程式的關鍵組件真的失敗時報告端點狀況良好。 如需詳細資訊，請參閱[健康情況端點監視控模式][health-endpoint-monitoring-pattern]。
