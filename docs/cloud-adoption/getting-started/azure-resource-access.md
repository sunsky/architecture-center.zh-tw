...
Azure 訂用帳戶類似於資源群組，它是一個邏輯建構，將資源群組及其資源群組在一起。 不過，Azure 訂用帳戶也與 Azure Resource Manager 所使用的控制項相關聯。 這代表什麼？ 讓我們看看 Azure Resource Manager，以了解它與 Azure 訂用帳戶之間的關聯性。
...
在 [Azure 如何運作？](what-is-azure.md)中，您已了解 Azure 包含「前端」與會協調 Azure 所有函式的多個服務。 這些服務的其中之一就是 [Azure Resource Manager](/azure/azure-resource-manager/)，這個服務會裝載用戶端用來管理資源的 RESTful API。
...
*圖 5.Azure 用戶端會連線到 Resource Manager RESTful API。*

雖然這些用戶端會使用 RESTful API 連線到 Resource Manager，但 Resource Manager 不包含直接管理資源的功能。 相反地，Azure 中的大多數資源類型都有自己的[**資源提供者**](/azure/azure-resource-manager/resource-group-overview#terminology)。

*
*圖 6.Azure 資源提供者。*

當用戶端要求管理特定資源時，Azure Resource Manager 會連線到該資源類型的資源提供者，來完成要求。 例如，如果用戶端要求管理虛擬機器資源，Resource Manager 會連線到 **Microsoft.compute** 資源提供者。

![](../_images/governance-1-15.png)
*圖 7.Resource Manager 會連線到 **Microsoft.compute** 資源提供者，來管理用戶端要求中指定的資源。*

Resource Manager 需要用戶端同時指定訂用帳戶和資源群組的識別碼，才能管理虛擬機器資源。

既然您已了解 Resource Manager 如何運作，讓我們回來討論 Azure 訂用帳戶如何與 Azure 資源管理員所使用的控制項相關聯。 在 Resource Manager 可以執行任何資源管理要求之前，會先檢查一組控制項。

第一個控制項是要求必須由已驗證使用者提出，而 Resource Manager 具有與 [Azure Active Directory (Azure AD)](/azure/active-directory/) 的信任關係，可以提供使用者身分識別功能。
...
最後一個控制項會檢查要求仍在與訂用帳戶相關聯的財務承諾之內。 例如，如果要求是要部署虛擬機器，Resource Manager 會確認訂用帳戶具有足夠的付款資訊。
...
既然您已了解如何在 Azure 中管理資源存取權，請繼續了解如何設計治理模型。

> [!div class="nextstepaction"]
> [雲端治理](../governance/overview.md)
