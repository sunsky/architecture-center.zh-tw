...
---
# <a name="deploy-a-container-based-devops-pipeline-for-modern-application-development-with-jenkins-and-azure-kubernetes-service"></a>為具有 Jenkins 和 Azure Kubernetes Service 的新式應用程式開發，部署容器型 DevOps 管線

此範例案例適用於想要藉由使用容器和 DevOps 工作流程讓應用程式開發現代化的企業。 在此解決方案中，Node.js Web 應用程式是由 Jenkins 建置，並且部署至 Azure Container Registry 和 Azure Kubernetes Service。 針對全域分散式資料庫層，使用 Azure Cosmos DB。 為了監視應用程式效能及進行疑難排解，Azure 監視器與 Grafana 執行個體和儀表板整合。
...
此解決方案涵蓋 Node.js Web 應用程式和資料庫後端的 DevOps 管線。 整個解決方案的資料流程如下所示：
...
* [Jenkins][jenkins] 是開放原始碼 Automation 伺服程式，可與 Azure 服務整合，以進行持續整合 (CI) 及持續部署 (CD)。 在此解決方案中，Jenkins 會根據對於原始檔控制的認可，協調新容器映像的建立，將這些映像推送至 Azure Container Registry，然後在 Azure Kubernetes Service 中更新應用程式執行個體。
...
為了監視您的應用程式效能並且報告問題，此解決方案將 Azure 監視器與 Grafana 合併到視覺效果儀表板。 這些工具可讓您監視可能需要程式碼更新 (後續可以使用 CI/CD 管線部署) 的效能問題，並且進行疑難排解。

負載平衡器是 Azure Kubernetes Service 叢集的一部分，它會將應用程式流量散佈到執行應用程式的一或多個容器 (Pod)。 在 Kubernetes 中執行容器化應用程式的這個方法，會為您的客戶提供高可用性基礎結構。

如需其他延展性主題，請參閱架構中心的[可用性檢查清單][availability]。
...
為了將攻擊量降至最低，此解決方案不會透過 HTTP 公開 Jenkins 虛擬機器執行個體。 針對需要您與 Jenkins 互動的任何管理工作，您使用 SSH 通道從本機機器建立安全的遠端連線。 Jenkins 和 Grafana 虛擬機器執行個體僅允許 SSH 公開金鑰驗證。 已停用密碼型登入。 如需詳細資訊，請參閱[在 Azure 上執行 Jenkins 伺服器](../../reference-architectures/jenkins/index.md)。

針對區隔認證和權限，此解決方案會使用專用的 Azure Active Directory (AD) 服務主體。 此服務主體的認證會在 Jenkins 中儲存為安全認證物件，因此它們不會直接公開，在指令碼或建置管線內也看不見。

如需設計安全解決方案的一般指引，請參閱 [Azure 安全性文件][security]。

### <a name="resiliency"></a>災害復原

此解決方案會針對您的應用程式使用 Azure Kubernetes Service。 復原元件內建於 Kubernetes，這些元件會監視及重新啟動有問題的容器 (Pod)。 您的應用程式與多個執行中 Kubernetes 節點合併，可以容許 Pod 或節點無法使用。
...
* 您需要 Azure Active Directory (AD) 服務主體以便驗證服務和資源。 您可以視需要使用 [az ad sp create-for-rbac][createsp] 來建立服務主體
    ...
    請記下此命令輸出中的 appId 和 password。 當您部署解決方案時，將這些值提供給範本。

若要使用 Azure Resource Manager 範本部署此解決方案，請執行下列步驟。
...
2. 等待 Azure 入口網站中的範本部署開啟，然後完成下列步驟：
   * 選擇 [新建] 資源群組，然後在文字方塊中提供名稱，例如 myAKSDevOpsSolution。
   * 從 [位置] 下拉式方塊選取區域。
...
為了探索執行此解決方案的成本，所有服務會在成本計算機中預先設定。 若要查看價格如何針對您的特定使用案例而變更，請變更適當的變數，以符合您預期的流量。 符合您預期的流量。
...
此解決方案使用 Azure Container Registry 和 Azure Kubernetes Service 來儲存及執行容器型應用程式。 Azure 容器執行個體也可以用來執行容器型應用程式，不需要佈建任何協調流程元件。 如需詳細資訊，請參閱 [Azure 容器執行個體概觀][azureaci-docs]。
