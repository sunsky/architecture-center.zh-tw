...
針對使用 [Azure App Service][app-service] 和 [Azure SQL Database][sql-db]，此參考架構會示範經過證實的作法。 [**部署這個解決方案**](#deploy-the-solution)。
...
永遠不要將密碼、存取金鑰或連接字串簽入原始檔控制。 而是會將這些項目作為參數傳遞至部署指令碼，將這些值儲存為應用程式設定。
...
此架構的範例 Resoure Manager 範本[可在 GitHub 上取得][paas-basic-arm-template]。
