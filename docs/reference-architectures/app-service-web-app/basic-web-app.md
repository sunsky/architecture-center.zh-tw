...
* **部署位置**。  [部署位置][deployment-slots]可讓您預先準備好部署，然後將其與正式環境部署交換。 這樣一來，您可以避免直接在正式環境中部署。 如需特定建議事項，請參閱[可管理性](#manageability-considerations)一節。
...
使用標準或進階層，因為其支援相應放大、自動調整規模和安全通訊端層 (SSL)。 每個服務層皆支援數個*執行個體大小* (因核心和記憶體數量不同而有所差異)。 建立方案之後，您可以變更服務層或執行個體的大小。 如需 App Service 方案的詳細資訊，請參閱 [App Service 價格][app-service-plans-tiers]。

您必須支付 App Service 方案中的執行個體費用，即使該應用程式已停用。 請務必刪除您不使用的方案 (例如測試部署所使用的方案)。
...
* 自動調整規則包含「冷卻」期間，也就是調整動作完成後，再次啟動新的調整動作之前所等候的間隔。 冷卻期間會讓系統穩定後，才能再次進行調整。 針對新增執行個體設定較短的冷卻期間，以及針對移除執行個體設定較長的冷卻期間。 例如，為新增執行個體設定 5 分鐘，但為移除執行個體設定 60 分鐘。 最好在負載過重時快速增加新執行個體來處理額外的流量，然後逐漸減少執行個體。
...
App Service 應用程式一律會有一個名為 `production` 的部署位置，代表實際執行的正式網站。 我們建議您為部署更新建立預備位置。 使用預備位置的優點包括：

* 您可以確認部署成功後，再交換至正式環境。
* 部署至預備位置，可確保所有執行個體在交換至正式環境之前就已準備就緒。 許多應用程式都有重要的暖機和冷啟動時間。

我們也建議您建立第三個位置來保存上一次的正確部署。 當您交換預備環境和正式環境之後，請將先前的正式環境部署 (現在處於預備環境) 移到「上一次的正確部署」位置。 這樣一來，如果您之後發現問題，您可以快速還原至上一次的正確版本。

![[1]][1]

如果還原至之前的版本，請確定任何資料庫的結構描述變更都可以與舊版相容。

請勿使用正式環境部署上的位置進行測試，因為同個 App Service 方案內的所有應用程式會共用相同的虛擬機器執行個體。 例如，負載測試可能會使實際執行的正式網站降級。 針對正式和測試，請建立不同的 App Service 方案。 透過將測試部署放在不同方案內，您可以使其與正式版本隔離。

### <a name="configuration"></a>組態
儲存作為[應用程式設定][app-settings]的組態設定。 在您的資源管理員範本中，或者使用 PowerShell 定義應用程式設定。 在執行階段上，應用程式設定可作為應用程式的環境變數。

永遠不要將密碼、存取金鑰或連接字串簽入原始檔控制，而是將這些設定作為參數傳遞至部署指令碼，將這些值儲存為應用程式設定。

依預設，當您交換部署位置時，應用程式設定也會進行交換。 如果正式環境和預備環境需要不同設定，您可以建立衷於位置且不要進行交換的應用程式設定。
...
每個部署位置都具有公用 IP 位址。 使用 [Azure Active Directory 登入][aad-auth]可保護非正式位置，因此只有您的開發和 DevOps 小組成員可以連線至這些端點。

### <a name="logging"></a>記錄
記錄絕對不能記下使用者的密碼，或其他可能可用來認可身分識別詐騙的資訊。 儲存之前請清除這些詳細資料。   
...
作為最佳安全性做法，您的應用程式應該透過重新導向 HTTP 要求，來強制執行 HTTPS。 您可以在您的應用程式中實作此項目，或如[針對 Azure App Service 中的應用程式啟用 HTTPS][ssl-redirect]中所述使用 URL 重寫規則。
...
* 容易設定。
* 不需要撰寫程式碼即可使用的簡單驗證情境。
* 支援委派授權，使用 OAuth 存取權杖來代表使用者使用資源。
* 提供內建權杖快取。
