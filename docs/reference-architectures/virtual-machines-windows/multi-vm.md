...
此參考架構顯示一組經過證實的作法，可用來在負載平衡器後方的擴展集中執行數個 Windows 虛擬機器 (VM)，以提升可用性和延展性。 此架構可用於任何無狀態的工作負載 (例如網頁伺服器)，而且是用於部署多層式架構應用程式的建置組塊。 [**部署這個解決方案**。](#deploy-the-solution)
...
此架構是根據[在 Azure 上執行 Windows VM][single vm] 建置的。 那裡的建議也適用於此架構。

在此架構中，會將工作負載散發到數個 VM 執行個體。 有個單一公用 IP 位址，而且會使用負載平衡器來將網際網路流量散發到 VM。 此架構可用於單層式架構應用程式，例如無狀態的 Web 應用程式。

此架構具有下列元件：

* **資源群組。** [*資源群組*][resource-manager-overview]可用來群組資源，讓它們可以受到存留期、擁有者及其他準則所管理。
* **虛擬網路 (VNet) 和子網路。** 在 Azure 中的每個 VM 都會部署到 VNet 中，而虛擬網路會進一步分成數個子網路。
* **Azure Load Balancer**。 [負載平衡器]會將連入網際網路要求散發到 VM 執行個體。 
* **公用 IP 位址**。 負載平衡器需要公用 IP 位址才能接收網際網路流量。
* **VM 擴展集**。 [VM 擴展集][vm-scaleset]是一組用來裝載工作負載且完全相同的 VM。 擴展集允許以手動方式或根據預先定義的規則，來相應縮小或放大 VM 數目。
...
根據預設，擴展集會使用「過度佈建」，這表示擴展集一開始會佈建比您所要求更多的 VM，然後刪除額外的 VM。 這可改善佈建 VM 時的整體成功率。 如果您未使用[受控磁碟](/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-managed-disks)，則建議每個儲存體帳戶不要超過 20 個已啟用過度佈建的 VM，或者不要超過 40 個已停用過度佈建的 VM。
...
如果您未使用擴展集，請考慮至少使用可用性設定組。 在可用性設定組中至少建立兩個 VM，以支援[適用於 Azure VM 的可用性 SLA][vm-sla]。 Azure 負載平衡器也會要求已負載平衡的 VM 屬於同一個可用性設定組。
...
將 VM 放置於同一個子網路中。 不要直接將 VM 公開至網際網路，而是改為每個 VM 提供私人 IP 位址。 用戶端會使用負載平衡器的公用 IP 位址來連線。

如果您需要登入負載平衡器後方的 VM，請考慮使用您可以登入的公用 IP 位址來新增單一 VM 作為防禦主機/Jumpbox。 然後從 Jumpbox 登入負載平衡器後方的 VM。 或者，基於相同目的，在負載平衡器中設定輸入 NAT 規則。 不過，當您裝載多層式架構工作負載或多個工作負載時，較好的解決方案是具備 Jumpbox。
...
針對每個 VM 建立個別的 Azure 儲存體帳戶來保存虛擬硬碟 (VHD)，以避免達到儲存體帳戶的每秒輸入/輸出作業 [(IOPS) 限制][vm-disk-limits]。

我們建議搭配使用[受控磁碟](/azure/storage/storage-managed-disks-overview)與[進階儲存體][premium]。 受控磁碟不需要儲存體帳戶。 您只需指定磁碟的大小和類型，並以高度可用的方式來部署它。
...
負載平衡器會使用[健康情況探查]來監視 VM 執行個體的可用性。 如果探查無法在逾時期間內連線到執行個體，負載平衡器就會停止將流量傳送到該 VM。 不過，負載平衡器將會繼續探查，而且如果 VM 再次變成可用，負載平衡器就會繼續將流量傳送到該 VM。
...
* 探查會從[已知][health-probe-ip]的 IP 位址 168.63.129.16 來傳送。 請確定您並未在任何防火牆原則或網路安全性群組 (NSG) 規則中封鎖往返此 IP 的流量。
...
如果有多個 VM，請務必將程序自動化，如此一來，它們就會更可靠且可重複。 您可以使用 [Azure 自動化][azure-automation]，來將部署、OS 修補及其他工作自動化。 [Azure 自動化][azure-automation]是以 Windows Powershell 為基礎且可針對這個使用的自動化服務。 範例自動化指令碼可從 TechNet 上的 [Runbook 資源庫]中取得。
...
適用於此架構的部署可在 [GitHub][github-folder] 上取得。 它會部署下列各項：
  ...
  * VM 擴展集，其中包含執行最新版 Windows Server 2016 Datacenter Edition 的 VM。 已啟用自動調整規模。
  * 位於 VM 擴展集前方的負載平衡器。
  * 含有傳入規則的 NSG，允許 HTTP 流量到 VM 擴展集。
...
1. 瀏覽至您在上述必要條件步驟中所下載存放庫的 `virtual-machines\multi-vm\parameters\windows` 資料夾。

2. 開啟 `multi-vm-v2.json` 檔案，然後在引號之間輸入使用者名稱和密碼，如下所示，然後儲存檔案。
