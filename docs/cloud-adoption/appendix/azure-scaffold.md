...
不再需要此種訂用帳戶激增情況。 使用[角色型存取控制](/azure/role-based-access-control/overview)，您可以將使用者指派給標準角色 (例如常見的「讀取者」和「寫入者」角色類型)。 您也可以定義自訂角色。
...
> [實作範例](azure-scaffold-examples.md)
