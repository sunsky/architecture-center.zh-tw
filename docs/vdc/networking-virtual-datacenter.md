...
下列雲端部署的**紅色方塊**顯示安全性缺口的範例。 **黃色方塊**顯示的工作負載最佳化的網路虛擬設備的空間。

[![0]][0]

虛擬資料中心 (VDC) 是為了支援企業工作負載而必須進行調整所產生的概念，而且還需要處理在公用雲端中支援大規模應用程式時所產生的問題。

VDC 實作不只代表雲端中的應用程式工作負載，也是網路、安全性、管理和基礎結構 (例如，DNS 和目錄服務)。 當越來越多企業工作負載移動到 Azure 時，務必考慮支援基礎結構和物件置於這些工作負載。 謹慎考量資源建構方式可以避免激增數百個「工作負載島」，而工作負載島必須使用獨立資料流程、安全性模型和相容性挑戰進行分開管理。
...
身分識別服務和目錄服務是內部部署和雲端中所有資料中心的一個關鍵層面。 身分識別是與 VDC 實作內服務存取和授權的所有層面有關。 為了協助確保只有授權使用者和處理序才能存取您的 Azure 帳戶和資源，Azure 會使用數種類型的認證進行驗證。 其中包括密碼以存取 Azure 帳戶、密碼編譯金鑰、數位簽章和憑證。 [Azure Multi-Factor Authentication (MFA)][MFA] 是存取 Azure 服務的額外安全層。 Azure MFA 提供增強式驗證與一系列簡單的驗證選項&mdash;通話、 簡訊或行動應用程式通知&mdash;及讓客戶選擇自己喜歡的方式。
...
[**ExpressRoute**][ExR] 是一種 Azure 連線服務，可讓您在 VDC 實作和任何內部部署網路之間建立私人連線。 ExpressRoute 連線不會經過公用網際網路，而且會提供更高的安全性、可靠性、速度 (最高 10 Gbps)，以及一致的延遲。 ExpressRoute 可輔助 VDC 的實作，因為 ExpressRoute 客戶可獲益於私人連線的相關合規性規則。 具有[ExpressRoute 直接][ExRD]，您可以直接連接到 100 的 Gbps 的 Microsoft 路由器較大的頻寬需求的客戶。
...
[VNet][VNet] 和 [VNet 對等][VNetPeering]是 VDC 實作內的基本網路連線服務。 VNet 保證 VDC 資源的自然隔離界限，而且 VNet 對等互連允許相同 Azure 區域內或甚至跨區域的不同 VNet 之間互相通訊。 在 VNet 內與 Vnet 之間的流量控制需要符合一組安全性規則，指定透過存取控制清單 ([網路安全性群組][NSG])，[網路虛擬設備] [ NVA]，和自訂路由表 ([UDR][UDR])。
...
單一 VDC 實作可以向上延展到大量支點; 不過，您也可以如同每個 IT 系統中，有平台限制。 中樞部署會繫結至具有限制的特定 Azure 訂用帳戶 (例如，VNet 對等數目上限 - 如需詳細資訊，請參閱 [Azure 訂用帳戶和服務限制、配額與限制][Limits])。 如果限制可能會產生問題，則將模型從單一中樞支點延伸到中樞和支點叢集，即可進一步向上延展架構。 一或多個 Azure 區域中的多個中樞可以使用 VNet 對等互連、ExpressRoute、虛擬 WAN 或站對站 VPN 互相連接。

[![2]][2]

導入多個中樞會增加系統的成本和管理工作。 它只會對齊的延展性、 系統限制或備援和區域性終端使用者效能或災害復原的複寫。 在需要多個中樞的案例中，所有中樞都應該致力於提供一組相同的易操作服務。

#### <a name="interconnection-between-spokes"></a>支點之間的互相連線

在單一支點中，就可以實作複雜的多層式工作負載。 可以在相同的 VNet 中使用子網路 (一層一個) 實作多層設定，以及使用 NSG 篩選流程。
...
支點也可以與作為中樞的支點互相連接。 這個方法會建立兩個層級階層： 較高的層級 （層級 0） 中支點變得較低支點 （層級 1） 階層的中樞。 實作 VDC 的支點需要將流量轉送到中央中樞，使流量可以傳輸到其內部部署網路或公用網際網路中的目的地。 具有雙層中樞的架構引進複雜路由，以移除簡單中樞支點關聯性的優點。
...
一般較好的做法是，存取權利和權限應該以群組為基礎。 處理群組，而不是個別使用者提供一致的方式來管理在團隊間、 簡化的存取原則的維護，並協助減少設定錯誤。 在適當群組中指派和移除使用者有助於保持特定使用者權限的最新狀態。
...
VDC 的設計，因此中央 IT 群組，管理中樞所建立的群組具有對應群組工作負載層級。 除了管理中樞資源之外，只有中央 IT 群組能夠控制外部存取，以及訂用帳戶的最上層權限。 也能夠控制資源和其獨立於中央 VNet 的權限的工作負載群組，是 IT。
...
通常，在 IT 中，環境 (或層) 是部署和執行多個應用程式的系統。 大型企業使用開發環境 （其中的變更會進行和測試） 和生產環境中 （使用者所使用）。 這些環境之間通常都會分隔成數個預備環境，以允許分階段部署 (推出)、測試以及在發生問題時復原。 部署架構極大，但通常仍會遵循開始開發環境 (DEV) 和結束生產環境 (PROD) 的基本程序。
...
- [**NSG**][NSG]. 網路安全性群組是做為篩選 IP 來源、 目的地 IP、 通訊協定、 IP 來源連接埠和 IP 目的地連接埠的流量的安全性規則的清單。 NSG 可以套用至子網路、與 Azure VM 建立關聯的虛擬 NIC 卡，或兩者。 若要實作中樞和支點中的正確流量控制，NSG 不可或缺。 NSG 所提供的安全性層級是您所開啟之連接埠和用途的功能。 客戶應該套用具有主機型防火牆 (例如 IPtables 或 Windows 防火牆) 的其他個別 VM 篩選。
...
[**UDR**][UDR]使用 UDR，客戶可以部署防火牆、IDS/IPS 和其他虛擬設備，並透過這些安全性設備來路由傳送網路流量，以強制執行安全性界限原則、稽核和檢查。 UDR 可以建立於中樞和輪輻中，保證流量傳輸到 VDC 實作所使用的特定自訂 VM、網路虛擬設備和負載平衡器。 若要保證該 Vm 產生的流量位於支點傳輸到正確的虛擬設備，UDR 需要設定在支點的子網路中，藉由將內部負載平衡器的前端 IP 位址設定為下個躍點。 內部負載平衡器會將內部流量分散到虛擬設備 (負載平衡器後端集區)。
...
[**Azure Load Balancer**][ALB] 提供高可用性層級 4 (TCP、UDP) 服務，可將連入流量分散到負載平衡組中所定義的服務執行個體。 可轉散發流量從前端端點 （公用 IP 端點或私用 IP 端點） 傳送到負載平衡器，使用或不到一組後端 IP 位址集區的位址轉譯 （範例包括網路虛擬設備或 Vm）。

Azure Load Balancer 可探查各種伺服器執行個體，健全狀況，當執行個體無法回應探查，負載平衡器會停止將流量傳送到狀況不良的執行個體。 在 VDC 中，外部負載平衡器會部署到中樞和輪輻中。 在中樞中，可使用負載平衡器來有效率地將流量路由至輪輻中的服務，而且在輪輻中，負載平衡器可用來管理應用程式流量。
...
- [事件記錄][NSGLog]會提供哪些 NSG 規則套用到以 MAC 位址為基礎的 VM 和執行個體角色的相關資訊。
...
**面向客戶的網站 (面向網際網路或內部)**：與網際網路互動的大部分應用程式都是網站。 Azure 提供在 IaaS VM 上或從 [Azure Web Apps][WebApps] 站台 (PaaS) 執行網站的功能。 Azure Web Apps 可與允許將 Web Apps 部署在輪輻網路區域中的 VNet 整合。 公開公用的網際網路端點，因為可透過私人非網際網路可路由傳送位址來自私人 VNet 存取資源，而不需要內部對向的網站。
...
目前為止，本文的重點在於單一 VDC 的設計，並描述構成復原的基本元件和架構。 Azure 的功能，例如 Azure 負載平衡器，Nva、 可用性設定組，擴展集，以及其他機制會導致系統，可讓您在您的生產服務中建置穩固的 SLA 層級。
...
- VNet 對等互連-VNet 對等互連可以跨區域連線中樞。
- ExpressRoute 私人對等互連，當在每個實作中 VDC 中樞連接到相同 ExpressRoute 電路。
- 多個 ExpressRoute 線路連接透過公司骨幹，您的多個 VDC 實作連接到 ExpressRoute 線路。
- 您的 VDC 實作每個 Azure 區域中的中樞區域之間的站對站 VPN 連線。
