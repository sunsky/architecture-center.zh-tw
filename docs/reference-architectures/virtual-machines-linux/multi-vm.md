...
此參考架構顯示一組經過證實的作法，可用來在負載平衡器後方的擴展集中執行數個 Linux 虛擬機器 (VM)，以提升可用性和延展性。 此架構可用於任何無狀態的工作負載 (例如網頁伺服器)，而且是用於部署多層式架構應用程式的建置組塊。 [**部署這個解決方案**。](#deploy-the-solution)
...
此架構是根據[在 Azure 上執行 Linux VM][single vm] 建置的。 那裡的建議也適用於此架構。
...
適用於此架構的部署可在 [GitHub][github-folder] 上取得。 其中包括 VNet、NSG，以及位於負載平衡器後方之擴展集中的三個 VM，如下所述：
