...
水平調整必須設計到系統裡。 例如，您可以將 VM 放在負載平衡器後方，以進行相應放大。 但是，集區中的每個 VM 都必須能夠處理任何用戶端要求，因此，應用程式必須是無狀態模式或是將狀態儲存在外部 (例如，儲存在分散式快取)。 受控 PaaS 服務通常都會內建水平調整和自動調整。 能夠輕鬆調整這些服務是使用 PaaS 服務的主要優勢之一。
