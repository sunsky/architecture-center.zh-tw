...
-   **Azure Active Directory (Azure AD)**。 [Azure AD][azure-ad] 支援使用者驗證，讓您可設定 SSO。 Azure AD [服務主體][service-principal]會透過[以角色為基礎的存取控制][rbac] (RBAC)，來定義工作流程中每個角色授權的原則和權限。 每個服務主體都會與 Jenkins 作業相關聯。
...
Jenkins 伺服器上安裝的 Azure AD 外掛程式會實作 SSO 驗證和授權。 SSO 可讓您在登入 Jenkins 伺服器時，使用來自 Azure AD 的組織認證來進行驗證。 在設定 Azure AD 外掛程式時，您可以指定使用者對 Jenkin 伺服器之授權存取權的層級。
...
-   [Azure 虛擬機器代理程式][configure-agent]外掛程式會使用 Azure Resource Manager (ARM) 範本，在 Azure 虛擬機器中建立 Jenkins 代理程式。
...
評估工作流程的可用性需求，以及 Jenkin 伺服器當機時如何復原 Jenkin 狀態。 若要評估您的可用性需求，請考量以下兩個計量：

-   復原時間目標 (RTO) 會指定在沒有 Jenkins 的情況您能運行多久的時間。

-   復原點目標 (RPO) 表示當進行中的中斷影響 Jenkins 時，您可以負擔多少的資料遺失量。

實際上，RTO 和 RPO 代表備援性和備份。 可用性不是硬體復原的問題 (那是 Azure 的問題)，而是要確保您會維護 Jenkins 伺服器的狀態。 此參考架構會使用 [Azure 服務等級協定][sla] (SLA)，其會保證單一虛擬機器 99.9 % 的執行時間。 如果這個 SLA 不符合您的執行時間需求，請確定您備妥災害復原的計畫，或考慮使用[多重主要 Jenkins 伺服器][multi-master]部署 (未涵蓋在本文件中)。
...
-   設定方法以保護對 Jenkin 伺服器的登入。 依預設 HTTP 並不安全，這個架構會使用 HTTP 並具有公用 IP。 請考慮在用於安全登入的 [Nginx 伺服器上設定 HTTPS][nginx]。
...
-   建立安全性設定檔，其會定義使用者、服務和管線代理程式執行其工作所需的資源，僅此而已。 當考慮到安全性設定時，此步驟變得很關鍵。
...
1.  在 Web 瀏覽器中開啟 [Jenkins 的 Azure 市集映像][azure-market]，然後從頁面左側選取 [立即取得]。
