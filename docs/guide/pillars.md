...
- 水平調整具有彈性。 您可以在負載增加時新增更多執行個體，也可以在負載較平靜的期間予以移除。
- 相應放大程序可自動觸發，不論是根據排程或因應負載的變化。 
- 相應放大的成本可能低於相應增加。 執行數個小型 VM 的成本會少於執行單一大型 VM。 
- 藉由新增備援，水平調整也可以提升復原功能。 如果執行個體停止運作，應用程式會繼續執行。
...
請一定要進行效能和負載測試，以找出這些潛在的瓶頸。 系統的具狀態部分 (例如資料庫) 是最常造成瓶頸的因素，必須謹慎設計才能進行水平調整。 因為，解決一個瓶頸可能會讓其他地方又出現瓶頸。
